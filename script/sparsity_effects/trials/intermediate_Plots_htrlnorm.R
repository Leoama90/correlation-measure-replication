...
# searches for the file in the project folder
...
# CLR Effects
p.htrlnorm.min <- df %>%
  reframe(ERR_CLR_MEAN  = mean(ERR_CLR), 
  ...
  scale_x_continuous(breaks = seq(-0.8, 0.8, 0.2), 
                     expand = c(0, 0)) +
...
example <- function(n, cor, meanlog_1, meanlog_2, sdlog_1, sdlog_2, phi_1, phi_2){
  ...
  random_HMP2 <- ToyModel::toy_model(n = n, 
                                     cor = diag(200), 
                                     M = 1,
