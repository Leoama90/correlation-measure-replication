...
#' @param n_zeroes integer. Number of off-diagonal entries to force to zero.
#'   Must be an even number, since zeroes are placed symmetrically in pairs
...
generate_matrix_with_zeroes <- function(n, n_zeroes, seed = NULL) {
  ...
  # stop if n_zeroes is not even, since zeroes must be placed in symmetric pairs
  if (n_zeroes %% 2 != 0) {
    stop("n_zeroes must be an even number, since zeroes are placed symmetrically")
  }
  ...
  if (n_zeroes / 2 > n_upper) {
    stop("requested n_zeroes (", n_zeroes, ") exceeds available off-diagonal ",
         "positions (", n_upper * 2, ")")
  }
  ...
  # randomly pick n_zeroes/2 distinct upper-triangle positions to zero out
  chosen <- upper_positions[sample(nrow(upper_positions), n_zeroes / 2), , drop = FALSE]
