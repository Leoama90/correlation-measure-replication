...
pall <- ggarrange(
  ...
  ggarrange(p.min, p.min.clr,
    common.legend = T, legend = "top", ncol = 2, labels = c("B", "C"),
    label.x = 0.06, label.y = 1.07
  ),
  # Bottom row: raw counts vs CLR for the max-correlation pair
  ggarrange(p.max, p.max.clr,
    common.legend = T, legend = "top", ncol = 2, labels = c("D", "E"),
    label.x = 0.06, label.y = 1.07
  ),
  nrow = 2
)
...
saveRDS(pall, here("01_from_paper", "sparsity_effects", "example_couple.rds"))
