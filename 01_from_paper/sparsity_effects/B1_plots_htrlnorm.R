...
df <- readRDS(here("01_from_paper", "sparsity_effects", "Sparsity_Effects_htrlnorm_2.rds")) %>%
  dplyr::select(-phi_2) %>%
  rename(phi = phi_1) %>%
...
myPalette <-
  c(RColorBrewer::brewer.pal(n = 11, "Spectral")) %>%
  rev() %>%
  c(., "#000000") %>%
...
saveRDS(p, here("01_from_paper", "sparsity_effects", "error_htlrlnorm.rds"))
