...
#   from a two-OTU htrlnorm/NorTA design as heatmaps of absolute error
#   (Normal vs. NorTA-CLR correlation) across input correlation and
#   zero-inflation levels (phi). Also generates illustrative scatterplot
#   examples (Normal / NorTA / NorTA-CLR) using simulated pairs of OTUs
#   embedded in a 200-OTU background community, with per-OTU parameters
#   drawn from empirical ranges estimated on real HMP2 microbiome data.
#
# Input:
...
#     healthy samples from subject 69-001
#
# Output:
#   - effects_htrlnorm_zerorand.png: three-panel heatmap of mean
#     absolute CLR error vs. input correlation and sparsity (phi min /
#     mean / max), saved under 01_from_paper/sparsity_effects/trials/
#   - examples_htrlnorm_zerorand.png: four-example, three-panel
#     scatterplot figure (Normal / NorTA / NorTA-CLR) illustrating the
#     effect of correlation sign and sparsity level, saved under the
#     same folder
#
# Packages used:
...
png(here("01_from_paper", "sparsity_effects", "trials", "effects_htrlnorm_zerorand.png"), width = 3600, height = 1500, res = 300)
print(ggarrange(
...
png(here("01_from_paper", "sparsity_effects", "trials", "examples_htrlnorm_zerorand.png"), width = 3600, height = 4800, res = 300)
