...
saveRDS(results_block, here("01_from_paper", "correlation_sparsity", "results_block.rds"))
...
# save results
saveRDS(results_gauss, here("01_from_paper", "correlation_sparsity", "results_gauss.rds"))
...
p_gauss <- results_gauss %>%
  ...
  labs(y = "MAE") +
  geom_hline(
    yintercept = median(results_gauss$ERR_CLR),
    linetype = "dotdash",
...
# generate the plot
png(
  filename = here("Plots", "correlation_density_biases.png"),
  width = 2800,
  height = 1500,
  res = 300
)
...
browseURL(list.files(
  path       = here(),
  pattern    = "correlation_density_biases.png",
  full.names = TRUE,
  recursive  = TRUE
))
