...
#   - SPIEC-EASI MB: sparse neighborhood selection via Meinshausen-Buhlmann regression
...
dir.create(here("01_from_paper", "tmp_files"), recursive = TRUE, showWarnings = FALSE)
...
colnames(adj_gl) <- rownames(adj_gl) <- colnames(res_gl$est$data)
adj_gl[adj_gl > 0] <- 1
adj_gl[adj_gl < 0] <- -1

# -------- SPIEC-EASI MB --------

# Sparse network estimation via Meinshausen-Buhlmann neighborhood selection
res_mb <- spiec.easi(
  data = otu_filt,
  method = "mb",
  lambda.max = 0.75,
...
df_gl <- tibble("method" = rep("clr", length(TRIU(res_clr))), "value" = TRIU(res_clr)) %>%
  rbind(tibble("method" = rep("gl", length(res_gl_clr)), "value" = res_gl_clr))

# -------- Plots --------

# creates a folder where to store all the outputs from this script
dir.create(
  here("outputs", "methods_comparison_outputs", "tmp_files"),
  recursive = TRUE,
  showWarnings = FALSE
)
...
set.seed(42)
p_graph_CLR <- as.grob(~ plot(
  g_clr,
  vertex.label = NA,
  vertex.color = colpal[taxa_filt[, "family"]],
  vertex.size = vertex_size,
  edge.color = ifelse(E(g_clr)$weight > 0, rgb(0, 0, 1), rgb(1, 0, 0)),
  edge.width = 0.5,
...
set.seed(42)
p_graph_glasso <- as.grob(~ plot(
  g_gl,
  vertex.label = NA,
  vertex.color = colpal[taxa_filt[, "family"]],
  vertex.size = vertex_size,
  edge.color = ifelse(E(g_gl)$weight > 0, rgb(0, 0, 1), rgb(1, 0, 0)),
  edge.width = 0.5,
  layout = LAYOUT_SIGNED(g_clr)
))
...
p_ven <- ggVennDiagram::ggVennDiagram(
  x = list(
    "GLASSO" = paste(as_edgelist(g_gl)[, 1], "-", as_edgelist(g_gl)[, 2], sep = ""),
    "CLR" = paste(as_edgelist(g_clr)[, 1], "-", as_edgelist(g_clr)[, 2], sep = "")
  ),
  label_alpha = 0
) +
...
png(
  here("outputs", "methods_comparison_outputs", "Methods_comparison.png"),
  width = 3600,
  height = 2400,
  res = 300
)
print(ggarrange(
  plotlist = list(p1, p2, p4, p_graph_CLR, p_graph_glasso, p_ven + theme(legend.position = "bottom")),
...
# MB network
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "graph_mb.png"),
  width = 1200,
  height = 1200,
  res = 200
)
...
plot(
  ...
  vertex.color = colpal[taxa_filt[, "family"]],
  vertex.size = vertex_size,
  edge.color = ifelse(E(g_mb)$weight > 0, rgb(0, 0, 1), rgb(1, 0, 0)),
  edge.width = 0.5,
...
# GLASSO network
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "graph_gl.png"),
  width = 1200,
  height = 1200,
  res = 200
)
...
# CLR network
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "graph_clr.png"),
  width = 1200,
  height = 1200,
  res = 200
)
...
# Venn diagram
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "ggvenn.png"),
  width = 1200,
  height = 1200,
  res = 200
)
...
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "colorLegend.png"),
  width = 1200,
  height = 1200,
  res = 200
)
...
# and CLR networks in a data frame
netw_info <- data.frame(
  "Edges" = sapply(list(g_mb, g_gl, g_clr), ecount),
  "Positive" = paste(
    100 * sapply(list(g_mb, g_gl, g_clr), function(x) round(sum(E(x)$weight > 0) / ecount(x), 2)),
    "%",
    sep = ""
  ),
  "Negative" = paste(
...
png(
  filename = here("outputs", "methods_comparison_outputs", "tmp_files", "table.png"),
  width = 1200,
  height = 1200,
  res = 300
)
print(
  ggplot() +
    ...
    annotation_custom(tableGrob(netw_info), xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf)
)
# closes the graphics device and saves the PNG file to disk
dev.off()
...
p_mb <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "graph_mb.png"))
p_gl <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "graph_gl.png"))
p_clr <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "graph_clr.png"))
p_leg <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "colorLegend.png"))
p_ven <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "ggvenn.png"))
p_inf <- cowplot::ggdraw() +
  cowplot::draw_image(here("outputs", "methods_comparison_outputs", "tmp_files", "table.png"))

# Arrange all panels into a 3-column grid and export as a single PNG
png(
  filename = here("outputs", "methods_comparison_outputs", "Graph_Comparison.png"),
  width = 1600,
  height = 1000,
  res = 300
)
