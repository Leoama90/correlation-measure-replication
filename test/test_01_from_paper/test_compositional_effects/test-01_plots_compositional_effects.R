...
#
# Purpose:
#   This script tests the fundamental features of the processing that
#   goes into 01_plots_compositional_effects.R, checking:
#       - compositional_effects_02.rds is at the expected project path
#       - df has the expected columns (d, pielou, ERR_L1, ERR_CLR) and
#         is not empty
#       - pielou is bounded within [0, 1]
#       - d contains only positive integers
#       - no NA values in the critical columns (d, pielou, ERR_L1, ERR_CLR)
#       - ERR_L1 and ERR_CLR are non-negative
#       - df_sort (nearest-diversity match per d, on the target Pielou
#         grid) has one row per (d, pielou_round) grid combination, with
#         no duplicated pairs and the exact expected set of grid values
#       - pielou_error is non-negative, and pielou_error_logical is a
#         clean logical column with no NA values
#       - df_percentiles has exactly one summary row per unique d
#       - the proportion of rows failing the pielou quality-control
#         threshold (df_control) stays below 10%
#
# Inputs:
#   - compositional_effects_02.rds
#
# Outputs:
#   - test results printed to the console (pass/fail for each check)
#
# Note:
#   This script does not source 01_plots_compositional_effects.R
#   directly. Instead, it locally re-implements the same grid-matching
#   and aggregation logic (building df_sort, df_percentiles, and
#   df_control from the saved compositional_effects_02.rds) so that
#   logic can be tested directly, without re-running the full upstream
#   simulation. Trade-off: this duplicates part of the original script's
#   processing logic here; if that logic changes in
#   01_plots_compositional_effects.R, this script must be updated to
